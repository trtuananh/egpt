...
from model import GPTConfig, GPT, DeepGPTConfig, DeepGPT, Encoder
...
print(f"Using context size: {block_size}")
...
# model init
model, checkpoint, model_args = DeepGPT.create(config=config, meta_vocab_size=meta_vocab_size)
...
# compile the model
if compile and os.name != 'nt' and torch.__version__ >= '2.0':
...
def get_continuous_batch(split, block_size=1024, batch_size=32, device='cpu', device_type='cpu'):
...
def get_batch(split, block_size, batch_size, device='cpu', device_type='cpu'):
...
if wandb_log and master_process:
    import wandb
    wandb.init(project=wandb_project, name=wandb_run_name, config=config.config_dict)
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if not eval_only and iter_num > 0 and always_save_checkpoint:
